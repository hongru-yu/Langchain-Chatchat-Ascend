...
def detect_device() -> Literal["mps", "cpu", "npu"]:
    try:
        import torch
        import torch_npu
        if torch_npu.npu.is_available():
...
def llm_device(device: str = None) -> Literal["mps", "cpu", "npu"]:
    device = device or LLM_DEVICE
    if device not in ["mps", "cpu", "npu"]:
        device = detect_device()
    return device


def embedding_device(device: str = None) -> Literal["mps", "cpu", "npu"]:
